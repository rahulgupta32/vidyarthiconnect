...
export default function StudentAIAssistant() {
  ...
  return (
    <div className="space-y-8">
      ...
      {notif && (
        <div className="bg-emerald-50 border border-emerald-250 text-emerald-600 dark:bg-emerald-950/20 dark:border-emerald-900/50 dark:text-emerald-400 text-xs rounded-xl p-4 font-semibold">
          {notif}
        </div>
      )}

      {notifError && (
        <div className="bg-rose-50 border border-rose-250 text-rose-600 dark:bg-rose-950/20 dark:border-rose-900/50 dark:text-rose-400 text-xs rounded-xl p-4 font-semibold">
          {notifError}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="space-y-4 lg:col-span-1">
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-4 rounded-2xl shadow-xs space-y-4">
                ...
                <button
                  ...
                  className={`w-full text-left p-2.5 rounded-xl text-xxs font-semibold transition border ${
                    activeConvId === conv.id 
                      ...
                      : "bg-slate-50 dark:bg-zinc-850 hover:bg-slate-100 border-transparent text-slate-700 dark:text-zinc-350"
                  }`}
                >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 bg-rose-50 hover:bg-rose-100 dark:bg-rose-950/20 dark:hover:bg-rose-950/40 text-rose-600 dark:text-rose-400 border border-rose-150 dark:border-rose-900/50 py-2 rounded-xl text-xxs font-bold transition"
            >
        ...
        <div className="lg:col-span-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-xs flex flex-col justify-between h-[550px] relative overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-4">
                ...
                >
                  <div className={`max-w-[85%] rounded-2xl p-4 text-xs leading-relaxed whitespace-pre-wrap ${
                    m.role === "USER"
                      ...
                      : "bg-slate-100 dark:bg-zinc-800 text-slate-800 dark:text-zinc-200 rounded-bl-none border border-slate-150 dark:border-zinc-700/50"
                  }`}>
                    {m.content}
                  </div>
                </div>
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-slate-50 dark:bg-zinc-850 text-slate-400 rounded-2xl rounded-bl-none p-3 text-xs flex items-center gap-2 border border-slate-100">
          ...
          {/* Quick Actions Panel */}
          {messages.length < 3 && (
            <div className="px-6 py-2 border-t border-slate-50 dark:border-zinc-850/80 bg-slate-50/50 dark:bg-zinc-900/50">
              ...
              <div className="flex flex-wrap gap-2">
                {quickActions.map((act, i) => (
                  <button
                    ...
                    className="flex items-center gap-1 bg-white dark:bg-zinc-800 hover:bg-indigo-50/40 border border-slate-200 dark:border-zinc-700 px-2.5 py-1.5 rounded-lg text-xxxxs font-bold text-slate-650 dark:text-zinc-350 transition"
                  >
          ...
          <div className="p-4 border-t border-slate-100 dark:border-zinc-800 bg-white dark:bg-zinc-900 flex gap-2 items-center">
            <input
              ...
              className="flex-1 bg-white dark:bg-slate-950 text-slate-900 dark:text-white placeholder:text-slate-500 dark:placeholder:text-slate-400 border border-slate-350 dark:border-slate-700 rounded-xl py-2.5 px-4 text-xs focus:outline-none focus:border-indigo-550 focus:ring-2 focus:ring-indigo-500/20"
            />
            <button
