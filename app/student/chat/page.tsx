...
export default function StudentChat() {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-slate-900/80 border border-slate-800 rounded-2xl shadow-xl flex flex-col h-[calc(100vh-12rem)] overflow-hidden">
      
      {/* Top Header */}
      <div className="bg-slate-950/40 px-6 py-4 border-b border-slate-800 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          <div className="bg-indigo-100 dark:bg-indigo-950 p-2.5 rounded-full text-indigo-600 dark:text-indigo-400">
            <User className="h-5 w-5" />
          </div>
          <div>
            <h2 className="font-bold text-sm text-white">Your Assigned Counselor</h2>
        ...
        <div className="text-xxs text-slate-400 font-semibold flex items-center gap-1 bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1">
              ...
              <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[70%] rounded-2xl p-4 shadow-xxs text-xs leading-relaxed ${
                  isMe 
                    ...
                    : "bg-slate-800 border border-slate-800 text-white rounded-bl-none"
                }`}>
      ...
      {/* Input Message Form */}
      <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-800 bg-slate-900/80 flex gap-2 flex-shrink-0">
        <input
          type="text"
          value={inputText}
          ...
          required
          className="flex-1 bg-slate-950 text-white placeholder:text-slate-400 border border-slate-800 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20"
        />
        <button
          type="submit"
