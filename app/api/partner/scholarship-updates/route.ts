...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validated = ScholarshipSchema.parse(payload);
    ...
    const submission = await db.partnerUpdateSubmission.create({
      data: {
        ...
        targetRecordId: scholarshipId || null,
    ...
    await logAudit(
      ...
      ip,
      `Submitted scholarship update proposal for university: ${partnerProfile.universityId}`
    );

    return NextResponse.json({ success: true, submission, message: "Scholarship update proposed. Pending admin review." });
  } catch (error: any) {
    console.error("Partner scholarship-update error:", error);
